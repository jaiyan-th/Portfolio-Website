...
function initializeNavigation() {
    ...
    window.addEventListener('scroll', function() {
        ...
        if (navbar) {
            if (scrollTop > 100) {
                navbar.classList.add('bg-white/90', 'dark:bg-black-bg/90');
                navbar.classList.remove('bg-white/80', 'dark:bg-black-bg/80');
            } else {
                navbar.classList.remove('bg-white/90', 'dark:bg-black-bg/90');
                navbar.classList.add('bg-white/80', 'dark:bg-black-bg/80');
            }
        }
...
function updateActiveNavLink() {
    ...
    navLinks.forEach(link => {
        ...
        link.classList.add('text-medium-gray', 'dark:text-light-gray');
        
        if (link.getAttribute('href') === `#${currentSection}`) {
            link.classList.remove('text-medium-gray', 'dark:text-light-gray');
...
function createProjectCard(project) {
    ...
    card.className = 'bg-white dark:bg-dark-surface rounded-lg border border-gray-200 dark:border-dark-border p-6 hover:border-accent-orange dark:hover:border-accent-orange transform hover:scale-101 transition-all duration-300 hover:shadow-xl';
    
    const techStackBadges = project.tech_stack.map(tech => 
        `<span class="px-2 py-1 bg-accent-orange/10 text-accent-orange text-xs rounded-full">${tech}</span>`
    ).join('');
            ...
            <div class="flex space-x-3">
                ${project.github_url ? `
                    <a href="${project.github_url}" target="_blank" 
                       class="text-gray-400 hover:text-accent-orange transition-colors duration-300">
